...
class IntersVivo {
    ...
    public createSystemInters(INTER_ID) {
        ...
        console.log('TYQSDK', 'VIVO 系统插屏广告初始化', INTER_ID)
        let id = INTER_ID;
        ...
        this.systemIntersAd.onLoad(() => {
            console.log('TYQSDK', 'VIVO 系统插屏广告加载完成')
            this.isLoadSystemInter = true;
        })

        //监听插屏广告错误
        this.systemIntersAd.onError(err => {
            console.log('TYQSDK', 'VIVO 系统插屏加载失败：' + JSON.stringify(err))
            this.isLoadSystemInter = false;
            if (err.errCode != 1008) {
                this.sysLoadErrNum++;
                console.log("TYQSDK", "VIVO 系统插屏加载错误次数:" + this.sysLoadErrNum);
        ...
        this.systemIntersAd.onClose(() => {
            console.log('TYQSDK', 'VIVO 系统插屏广告关闭');
    ...
    public showSystemInters() {
        if (this.systemIntersAd) {
            console.log("TYQSDK", "VIVO showSystemInters==================");
            this.systemIntersAd.show().then(() => {
                console.log("TYQSDK", "VIVO 系统插屏广告展示成功==================");
            }).catch((err) => {
                console.log("TYQSDK", "VIVO 系统插屏展示错误:" + JSON.stringify(err));
                IntersController.getInstance().intersNowTime = 0;
            })
        } else {
            console.log("TYQSDK", "VIVO 未创建系统插屏广告");
            return;
        }
    }
}
