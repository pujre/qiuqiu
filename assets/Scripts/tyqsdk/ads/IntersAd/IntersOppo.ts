...
class IntersOppo {
  ...
  public createSystemInters(INTER_ID) {
    ...
    console.log('TYQSDK', 'OPPO 插屏广告初始化', INTER_ID)
    var self = this;
    ...
    this.systemIntersAd.onLoad(() => {
      console.log('TYQSDK', 'OPPO 插屏广告加载完成')
      this.isLoadSystemInter = true;
    })

    //监听插屏广告加载错误
    this.systemIntersAd.onError(err => {
      console.log('TYQSDK', 'OPPO 插屏加载失败：' + JSON.stringify(err))
    })

    //监听插屏广告展示
    this.systemIntersAd.onShow(err => {
      console.log('TYQSDK', 'OPPO 插屏展示错误：' + JSON.stringify(err));
    });
