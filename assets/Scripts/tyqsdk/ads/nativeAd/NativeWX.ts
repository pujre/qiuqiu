    ...
    */
    ...
    public createNativeAd(ID) {
        ...
        if (!SdkTools.getInstance().isversionNewThanEngineVersion("2.11.1")) {
            console.log("TYQSDK", "WX 平台版本过低,不能创建原生广告");
            return;
        }

        this.NativeID = ID;

        console.log("TYQSDK", "WX 原生广告初始化", ID);
        ...
        this.nativeAd.onLoad(() => {
            console.log("TYQSDK", "WX 原生ICON加载成功");
            this.isLoadIconNative = true;
        });

        // 监听原生广告错误
        this.nativeAd.onError((err) => {
            console.log("TYQSDK", "WX 原生ICON加载失败:" + JSON.stringify(err));
            this.isLoadIconNative = false;
        });

        // 监听原生广告关闭
        this.nativeAd.onClose(() => {
            console.log("TYQSDK", "WX 手动关闭原生ICON 30s后再次刷新");
    ...
    public showNativeIcon(x, y) {
        ...
        setTimeout(() => {
            console.log('TYQSDK', 'WX showNativeIcon===============================');
            this.nativeAd.show();
        }, 500);

        this.updateICON =
            setInterval(() => {
                console.log("TYQSDK", "WX 刷新原生ICON广告================");
    ...
    public hideNativeIcon() {
        ...
        if (this.nativeAd) {
            console.log("TYQSDK", "WX hideNativeIcon===============================');");
            // this.nativeAd.offLoad();
            // this.nativeAd.offError();
            this.nativeAd.destroy();
        } else {
            console.log("TYQSDK", "WX 不存在原生广告实例");
            return;
        }
    }
}
