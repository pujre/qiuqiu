    ...
    */
    ...
    public createBlock(ID) {
        ...
        if (!SdkTools.getInstance().isversionNewThanEngineVersion("2.9.2")) {
            console.log("TYQSDK", "WX 平台版本过低,不能创建积木广告");
            console.log("TYQSDK", "当前版本：" + wx.getSystemInfoSync().SDKVersion);
            return;
        }

        this.ID_BlockID = ID;

        console.log('TYQSDK', 'WX 格子(积木)广告初始化', ID);
        ...
        this.blockAd.onLoad(function () {
            console.log("TYQSDK", "WX 格子(积木)广告加载成功");
            self.isLoadBlock = true;
        })

        // 监听积木广告错误
        this.blockAd.onError(function (err) {
            console.log("TYQSDK", "WX 格子(积木)广告加载失败:" + JSON.stringify(err));
        })
    ...
    public showBlock(type: string, x: number, y: number, blockSize: number) {
        ...
        setTimeout(() => {
            console.log('TYQSDK', 'WX showBlock===============================');
            this.blockAd.show();
        }, 500);

        this.updateBlock =
            setInterval(() => {
                console.log("TYQSDK", "WX 刷新积木广告================");
    ...
    public hideBlock() {
        ...
        if (this.blockAd) {
            console.log("TYQSDK", "WX hideBlock==========================");
            this.blockAd.destroy();
        } else {
            console.log("TYQSDK", "WX 不存在积木广告");
            return;
        }
    }
