    ...
    */
    ...
    public createBlock(ID) {
        ...
        if (!SdkTools.getInstance().isversionNewThanEngineVersion("1.15.0")) {
            console.log("TYQSDK", "QQ 平台版本过低,不能创建积木广告");
            return;
        }

        this.ID_BlockID = ID;

        console.log('TYQSDK', 'QQ 积木广告初始化', ID);
        ...
        this.blockAd.onLoad(function () {
            console.log("TYQSDK", "QQ 积木广告加载成功");
            self.isLoadBlock = true;
        })

        // 监听积木广告错误
        this.blockAd.onError(function (err) {
            console.log("TYQSDK", "QQ 积木广告加载失败:" + JSON.stringify(err));
            if (err.errCode == 1004) {
                console.log("TYQSDK", "QQ 积木广告列表为空");
            }
        })
    ...
    public showBlock(type: string, x: number, y: number, blockSize: number) {
        ...
        if (qq.getSystemInfoSync().version == "8.3.6") {
            console.log("TYQSDK", "QQ 平台版本过低,不展示积木广告==============");
            return;
        }
        else {
        ...
        setTimeout(() => {
            console.log('TYQSDK', 'QQ showBlock===============================');
            this.blockAd.show();
        }, 500);

        this.updateBlock =
            setInterval(() => {
                console.log("TYQSDK", "QQ 刷新积木广告================");
    ...
    public hideBlock() {
        ...
        if (this.blockAd) {
            console.log("TYQSDK", "QQ hideBlock==========================");
            this.blockAd.destroy();
        } else {
            console.log("TYQSDK", "QQ 不存在积木广告");
            return;
        }
    }
