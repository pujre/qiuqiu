...
class VideoTiktok {
     ...
     * 视频是否展示中
     */
    public videoOnShow: boolean = false;
    ...
    public createVideoAd(ID) {
        ...
        console.log('TYQSDK', 'Tiktok 视频广告初始化', ID)
        ...
        //监听视频广告加载完成
        this.videoAd.onLoad(() => {
            console.log('TYQSDK', 'Tiktok 视频广告加载完成')
            this.isLoadVideo = true;
        })

        //监听视频广告加载出错
        this.videoAd.onError(err => {
            console.log('TYQSDK', 'Tiktok 视频广告加载失败 ：' + JSON.stringify(err))
        ...
        this.videoAd.onClose(res => {
            setTimeout(() => {
                if (res.isEnded) {
                    console.log('TYQSDK', 'Tiktok 激励视频广告完成，发放奖励')
                    ...
                    console.log('TYQSDK', 'Tiktok 激励视频广告取消关闭，不发放奖励')
                    if (this.videoCallBack) {
    ...
    public showVideo(callback) {
        let self = this;
        if (this.videoOnShow) {
            console.log("TYQSDK", "视频正在播放中,请勿多次点击！");
            return;
        }
        ...
        if (this.videoAd) {
            self.videoAd.show()
                .then(() => {
                    console.log("TYQSDK", "Tiktok 激励视频广告显示成功");
                })
                .catch(err => {
                    console.log("TYQSDK", "Tiktok 激励视频广告组件出现问题", JSON.stringify(err));
                    // 可以手动加载一次
                    self.videoAd.load().then(() => {
                        console.log("TYQSDK", "Tiktok 激励视频广告手动加载成功");
                        // 加载成功后需要再显示广告
                        self.videoAd.show().catch(err => {
                            console.log("TYQSDK", "Tiktok 激励视频广告播放失败", JSON.stringify(err));
