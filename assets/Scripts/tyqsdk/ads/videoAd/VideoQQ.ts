...
class VideoQQ {
    ...
    public createVideoAd(ID) {
        ...
        console.log('TYQSDK', 'QQ 视频广告初始化', ID)
        ...
        //监听视屏广告加载完成
        this.videoAd.onLoad(() => {
            console.log('TYQSDK', 'QQ 视屏广告加载完成')
            self.isLoadVideo = true;
        })

        //监听视屏广告加载出错
        this.videoAd.onError(err => {
            console.log('TYQSDK', 'QQ 视屏加载失败 ：' + JSON.stringify(err))
        ...
        this.videoAd.onClose(res => {
            setTimeout(() => {
                if (res.isEnded) {
                    console.log('TYQSDK', 'QQ 激励视频广告完成，发放奖励')
                    if (this.videoCallBack) {
                        this.videoCallBack(true);
                        this.videoAd.load();
                    }
                } else {
                    console.log('TYQSDK', 'QQ 激励视频广告取消关闭，不发放奖励')
                    if (this.videoCallBack) {
    ...
    public showVideo(callback) {
        if (this.videoAd) {
            ...
            console.log('TYQSDK', 'QQ showVideo ========================')
            this.videoAd.show();
